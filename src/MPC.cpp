...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    // Reference State Cost //TODO add more costs if needed
    ...
    //Minimise the value difference between sequential actuations
    ...
    for (size_t t = 1; t < N; t++) {
      ...
      // Actuator values at t
      AD<double> delta0 = vars[delta_start + t -1];
      AD<double> a0 = vars[a_start+t -1];

      //TODO This is for a line, check stuff for a Third order equation
      // y = a0x^3+a1x^2+a2x+a3
      AD<double> f0 = coeffs[0]*CppAD::pow(x0,3)+ coeffs[1]*CppAD::pow(x0,2)\
                      + coeffs[2]*x0 + coeffs[3];
      //slope = atan(dy/dx) = atan(3*a0*x² + 2*a1*x+a2)
      AD<double> psides0 = CppAD::atan((coeffs[0]*3*(CppAD::pow(x0,2)) \
                            + coeffs[1]*2*x0 +coeffs[2]));
    ...
    }//end of for loop for filling the constraints
  }
};
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  // All all non-actuators upper and lowerlimits are set
